...
if(isset($_GET['id'])) {
	...
	{	
?>
	<p>Viewing Video:<?php echo $result_row[3].$result_row[2];?></p>
...
<?php } 
	...
	<?php echo '<a href="MeTubeSearch.php?q='.$tag[1].'">'.$tag[1].'</a>'; while ($tag = mysql_fetch_row($tags)) { echo ', '.'<a href="MeTubeSearch.php?q='.$tag[1].'">'.$tag[1].'</a>'; } ?> <br>
		Category: <?php echo '<a href="MeTubeSearch.php?q='.$result_row[7].'">'.$result_row[7].'</a>'; ?> <br>
<?php if($result_row[1] == $_SESSION['userID']) { 
	...
	<h3>Edit Metadata</h3>
			...
			<textarea name="tags" rows="4" cols="30"><?php echo $tag[1]; while ($tag = mysql_fetch_row($tags)) { echo ', '.$tag[1]; } ?></textarea><br>
				Category:<br>
...
?>
	...
	echo "<h2>Comments are disabled for this media</h2>";
		...
		<div id="comments">
			...
			?>
				<tr><td>
					<b><?php echo '<a href="MeTubeAccount.php?id='.$result_row[2].'">'.$result_row[0].'</a>: '; ?></b>
					<?php echo $result_row[1]; 
					if($result_row[2] == $_SESSION['userID']) {?>
					<form class="delcomment" action="delcomment.php" method="post">
	...
	<?php } ?>
		<!-- Since there will be no recommended algorthm I figured we could no videos on the side, or put 		videos from the same category. In which case: <div id="recommended"></div> -->
